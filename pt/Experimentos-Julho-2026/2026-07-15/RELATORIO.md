...
> **Data:** 15/07/2026 | **Container:** stellar/quickstart:testing (testnet) | **Ledger:** ~3,629,501
...
### 9.1 Bancos de Dados — Diagrama ER

![Diagrama ER de Bancos](../Docker-Stellar/diagramas/png/01-bancos.png)

Diagrama entidade-relacionamento dos bancos: PostgreSQL (Horizon), SQLite (Node e Captive Core) e Bucket Lists, com tamanhos e principais tabelas.
...
### 9.2 Tabelas do Horizon — Mindmap

![Mindmap de Tabelas](../Docker-Stellar/diagramas/png/02-tabelas.png)

Mindmap das 33 tabelas do PostgreSQL organizadas por categoria: Estado Atual, Histórico, Auxiliares e Junção.

```mermaid
mindmap
  root((Tabelas Horizon<br/>33 tabelas))
    Estado_Atual
...
### 9.3 Serviços do Container

![Diagrama de Serviços](../Docker-Stellar/diagramas/png/03-servicos.png)

Arquitetura de serviços dentro do container: Supervisor gerenciando PostgreSQL, Stellar Core Node, Horizon e Nginx, com Captive Core como subprocesso do Horizon.

```mermaid
graph TB
    subgraph Container["Container: stellar-testnet (172.18.0.2)"]
        subgraph Supervisionados["Gerenciados pelo Supervisor"]
            direction TB
        ...
        CC["Captive Core<br/>PID 3985<br/>~2.9 GB RAM<br/>:11725 P2P<br/>:11726 HTTP<br/>Subprocesso do Horizon"]
    end
    ...
    HZ -->|subprocess| CC
    HZ -->|"consulta SQL<br/>DATABASE_URL"| PG
    HZ -->|"consulta HTTP<br/>STELLAR_CORE_URL"| CC
    ...
    style Supervisionados fill:#16213e,color:#fff,stroke:#0f3460
...
### 9.4 Fluxo de Sincronização (Sequência)

![Diagrama de Sincronização](../Docker-Stellar/diagramas/png/04-sincronizacao.png)

Fluxo completo de sincronização em 5 fases: Descoberta, Download de State, Checkpoints, Ingestão do Horizon e Operação Contínua.
...
sequenceDiagram
    ...
    rect rgb(20, 30, 50)
        Note over CN,HZ: FASE 1: DESCOBERTA
    ...
    rect rgb(30, 50, 30)
        Note over CN,HZ: FASE 2: DOWNLOAD DE STATE
        CN->>HA: Download state file (history-*.json)
        CN->>HA: Download buckets (36-37 arquivos .xdr)
    ...
    rect rgb(50, 50, 20)
        Note over CN,HZ: FASE 5: OPERACAO CONTINUA
...
### 9.5 Hierarquia da Bucket List

![Diagrama da Bucket List](../Docker-Stellar/diagramas/png/05-bucketlist.png)

Hierarquia dos 11 níveis da Bucket List (0-10) com tamanhos, processo de merge e conexão com History Archives.

```mermaid
graph TB
    subgraph BUCKET_LIST["Bucket List (11 niveis: 0 a 10)"]
        L0["Nivel 0<br/>~25 KB<br/>2-4 buckets<br/>Ledgers recentes"]
        L1["Nivel 1<br/>~47 KB<br/>2-4 buckets"]
        L2["Nivel 2<br/>~1.6 MB<br/>2-4 buckets"]
        L3["Nivel 3<br/>~1.1 MB<br/>2-4 buckets"]
        L4["Nivel 4<br/>~1.8 MB<br/>2-4 buckets"]
        L5["Nivel 5<br/>~6.5 MB<br/>2 buckets"]
        L6["Nivel 6<br/>~40 MB<br/>2 buckets"]
        L7["Nivel 7<br/>~103 MB<br/>2 buckets"]
        L8["Nivel 8<br/>~211 MB<br/>2 buckets"]
        L9["Nivel 9<br/>~629 MB<br/>2 buckets"]
        L10["Nivel 10<br/>~822 MB<br/>2 buckets<br/>Snapshot completo"]
    end

    subgraph ARQUIVOS["Arquivos por Bucket"]
        XDR["bucket-{sha256}.xdr<br/>Dados serializados XDR"]
        INDEX["bucket-{sha256}.index<br/>Indice para lookup<br/>busca binaria"]
    end

    LEDGER["NOVO LEDGER<br/>atualizacoes no estado"] -->|"entra como<br/>bucket nivel 0"| L0
    L0 -->|"merge periodico<br/>a cada 64 ledgers"| L1
    L1 -->|merge| L2
    L2 -->|merge| L3
    L3 -->|merge| L4
    L4 -->|merge| L5
    ...
    L0 -.->|"contem .xdr + .index"| ARQUIVOS
    L1 -.-> ARQUIVOS

    HISTORY["History Archives<br/>history.stellar.org"] -->|"download<br/>catchup inicial"| L10
    L10 -->|"upload<br/>novo snapshot"| HISTORY
```

### 9.6 Conexões de Rede Externas

![Diagrama de Conexões de Rede](../Docker-Stellar/diagramas/png/06-conexoes.png)

Conexões de rede do container com validadores SDF, peers P2P, History Archives e o usuário local.
...
graph LR
    subgraph INTERNET["Internet"]
        ...
        PEERS["Peers aleatorios<br/>~10 conexoes P2P<br/>13.223.55.158<br/>44.204.146.210<br/>3.85.105.105<br/>44.206.255.84<br/>44.213.67.110<br/>98.91.174.101<br/>141.98.219.89<br/>54.159.155.163"]
    end
    ...
    subgraph HOST["Host Windows"]
        USER["Usuario<br/>localhost:8000"]
    end
...
### 9.7 Arquitetura Geral de Serviços (Visão Simplificada)
...
### 9.8 Fluxo de Inicialização (Startup)
