#!/usr/bin/env bash
# Monitoramento de queries SQL em todos os bancos por 90 segundos
# Executado: $(date -u +"%Y-%m-%dT%H:%M:%SZ")

OUTDIR="/tmp/sql_monitor"
mkdir -p "$OUTDIR"

echo "=========================================="
echo "INICIO DO MONITORAMENTO SQL (90 segundos)"
echo "Data: $(date -u)"
echo "=========================================="

# =========================================================
# SNAPSHOT 1: ANTES (baseline)
# =========================================================

echo "[1/5] Coletando snapshot BEFORE..."
...
# PostgreSQL - pg_stat_activity (queries ativas)
...
echo "Snapshot BEFORE coletado."

# =========================================================
# MONITORAMENTO ATIVO (5 em 5 segundos por 90s)
# =========================================================

echo "[2/5] Iniciando amostragem a cada 5s por 90s..."
echo "" > "$OUTDIR/pg_activity_samples.sql"

for i in $(seq 1 18); do
    sleep 5
    
    # Amostra unix timestamp
    TS=$(date -u +"%H:%M:%S")
    
    echo "--- Amostra $i / 18 (T=${TS}) ---" >> "$OUTDIR/pg_activity_samples.sql"
...
echo "Amostragem concluida."

# =========================================================
# SNAPSHOT 2: DEPOIS (after 90s)
# =========================================================

echo "[3/5] Coletando snapshot AFTER..."
...
echo "Snapshot AFTER coletado."

# =========================================================
# SQLITE: schema e estatisticas
# =========================================================

echo "[4/5] Coletando dados dos SQLites..."
...
echo "Dados dos SQLites coletados."

# =========================================================
# RESUMO
# =========================================================

echo "[5/5] Gerando resumo..."
echo ""
echo "=========================================="
...
echo ""
echo "--- PostgreSQL Estatisticas de Tabelas ---"
psql -h localhost -U stellar -d horizon -c "
SELECT 'Tabelas com mais inserts' as tipo;
SELECT
...
LIMIT 15;
" 2>/dev/null || echo "(delta nao disponivel - primeira leitura)"

echo ""
echo "--- Top 10 Queries por Tempo Total ---"
psql -h localhost -U stellar -d horizon -c "
SELECT
...
ORDER BY total_exec_time DESC
LIMIT 15;
" 2>/dev/null || echo "(pg_stat_statements vazio)"

echo ""
echo "--- Distribuicao por Tipo de Comando ---"
psql -h localhost -U stellar -d horizon -c "
SELECT
    CASE
